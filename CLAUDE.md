...
1. **Unsaturated-probe protocol for mega-hubs** (`math/direction1_megahub_limitation.md`). The N=2000 failure cause is now ISOLATED by diagnostic: 2 mega-hubs (|supp|≈1703) are driven into saturation by the pools, fail the validity filter, get skipped (zeroed W_hat column), and their silencing causes ~0.10 of the 0.14 divergence. Substituting W_TRUE for just those 2 columns recovers div 0.14→0.042. **The fix is a probe-design change:** lower-amplitude / sparse-targeted probes that keep mega-hubs in their non-saturated [0.05, 0.85] range so they yield valid observations. Then re-test N=2000. The residual 0.042 is ordinary bulk-recon error (Pearson 0.29 at low K) — fixed by more pool trials. Neither is a physics barrier or an algorithm gap; both are protocol/scaling refinements.
