...
**The end-to-end functional teleportation pipeline is demonstrated on SPARSE connectomes** (mean in-degree ≲ 10): C. elegans (N=300) and Drosophila mushroom body subsets up to N=2000, all on real biological data, all 5 behavioral tests PASS. **It FAILS on dense networks** — a whole-FlyWire top-5000-by-degree subset (mean in-degree 47) gives behavioral divergence ~0.68 (`math/direction1_density_limitation.md`). Mammalian cortex (mean in-degree ~7000) is far denser still. The information-theoretic content is low (rate-distortion holds) but the per-neuron reconstruction's accuracy (Pearson ~0.35 on real data) is insufficient for dense networks — a real, unsolved algorithm-accuracy problem. All five "viable mechanisms" surveyed at project start: four (quantum approaches) closed with negative verdicts; the fifth (Direction 1) is the viable path, demonstrated for sparse neural tissue, NOT yet for dense cortex.
...
1. **Dense-network reconstruction accuracy** (`math/direction1_density_limitation.md`). THE top open problem. The pool-stim pipeline passes on sparse connectomes (mean in-degree ≲ 10) but fails on dense ones (mean in-degree 47 → behavioral div 0.68). Mammalian cortex is far denser (~7000). The per-neuron support-aware ridge regression only reaches Pearson ~0.35 on real connectome data; for sparse networks this still gives behavioral PASS (errors average out over few input terms), but dense networks amplify the same per-weight error into catastrophic behavioral divergence. The fix must substantially improve reconstruction accuracy — candidate: multi-task low-rank reconstruction (SVT solver exists in `run_megahub_svt.py`) applied per cell type, exploiting the measured low d_eff. This is an algorithm-accuracy problem, not a physics barrier, but genuinely unsolved.
   (Mega-hub saturation issue from earlier is RESOLVED — mixed-amplitude ladder + coverage K·M≥3N; N=2000 sparse PASSES.)
