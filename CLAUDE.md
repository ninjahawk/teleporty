...
1. **N=2000 failure — isolate the cause** (`math/direction1_megahub_limitation.md`). THE top open problem. The pipeline fails at the N=2000 FlyWire mushroom body subset (div=0.13, flat across coverage). The cause is NOT definitively isolated: a synthetic mega-hub test did NOT reproduce it (per-neuron strong-ridge handles K≪|supp| fine without skipping). Leading candidate: the 81 "skipped" neurons (vj.sum()<5 → zeroed W_hat columns → silenced neurons distorting behavior). Next step: re-run N=2000 with W_hat saved, then substitute W_TRUE for skipped columns and re-test behavior — this isolates skipped-neuron contribution vs mega-hub contribution. SVT multi-task solver is implemented (`run_megahub_svt.py`) and ready if mega-hubs turn out to be the cause. Likely the real fix is improving probe design so fewer neurons are unobservable (skipped).
